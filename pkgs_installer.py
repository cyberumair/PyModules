...
def install_pkgs(pkgs: list):
    ...
    if len(pkg_done) == len(pkgs):
        ...
        if not_installed_pkgs == 0:
            print(
                f"{Fore.GREEN}✅ Done, All dependencies installed Successfully!{Style.RESET_ALL}\n"
            )
        else:
            print(
                f"{Fore.RED}❗ Something Went Wrong, {not_installed_pkgs} dependencie(s) are (is) Not installed.{Style.RESET_ALL}"
            )
            print(
                f"\t{Fore.YELLOW}Exiting, Try running the program again...{Style.RESET_ALL}\n"
            )
            exit()
